...
# -*- coding: utf-8 -*-
"""
NanoGPT-style experiments (Tiny Shakespeare / TinyStories):
AGOP Spectral Curriculum (AGOP-SC) vs baselines (random, loss-curriculum, self-paced, anti-curriculum).

Key new features (v2):
- dataset switch: tinyshakespeare (char) or tinystories (byte or GPT-2 BPE)
- AGOP Top-K projection for large vocab (e.g., BPE 50k)
- diversity mixing for both AGOP and baselines to avoid degenerate sampling
- optional warm-start before enabling AGOP-SC

All artifacts saved under /experiments/nanogpt_curriculum/ (fallback: ./experiments/nanogpt_curriculum/).
"""

import os, math, json, argparse, time, sys
...
def get_exp_dir():
    ...
    try:
        ...
        # test write
        p = os.path.join(preferred, ".write_test")
...
def build_tinystories_dataset(tokenizer: str="byte", max_chars:int=2000000,
                              val_ratio:float=0.01, seed:int=1337):
    """
    tokenizer: "byte" or "gpt2"
    returns: train_tokens (np.int64), val_tokens (np.int64), vocab_size (int)
    """
    try:
        from datasets import load_dataset
    except Exception as e:
        raise RuntimeError("Please `pip install datasets` to use --dataset tinystories") from e
    ...
    if tokenizer == "gpt2":
        try:
            ...
            raise RuntimeError("Please `pip install tiktoken` to use --tokenizer gpt2") from e
    ...
    train_data = ids[:n]
    val_data = ids[n:]
    print(f"TinyStories tokens: train={len(train_data)}, val={len(val_data)}, vocab={vocab_size}, tokenizer={tokenizer}")
    return train_data, val_data, vocab_size
...
class AGOPSpectralSelector:
    ...
    def step_select(self, model: nn.Module, x_cand: torch.Tensor, y_cand: torch.Tensor,
                    gamma: float=0.0, topk_logits: Optional[int]=None):
        with torch.no_grad():
            logits = model(x_cand)
        ...
        return order, {"scores":scores, "align":a, "novel":n, "w":w, "c":c, "loss_seq":loss_seq, "phi":phi}
...
def train_one_method(method: str, train_data: np.ndarray, val_data: np.ndarray, vocab_size: int, args) -> Dict:
    ...
    config = GPTConfig(vocab_size=vocab_size, block_size=args.block_size,
    ...
    best_val = 1e9; step_to_threshold = None; threshold = args.ppl_threshold
    ...
    for step in range(1, args.steps + 1):
        ...
        if method == "random":
            ...
            if step < args.agop_start_step:
                ...
                order, info = selector.step_select(model, x_cand, y_cand, gamma=args.gamma, topk_logits=topk)
        ...
        logits = model(x_cand[select_idx]); y_sel = y_cand[select_idx]
        ...
        if step % args.eval_interval == 0 or step == 1 or step == args.steps:
            ...
            if args.ppl_threshold > 0 and step_to_threshold is None and val_ppl <= args.ppl_threshold:
                step_to_threshold = step

    ckpt = {"model": model.state_dict(),"config": config.__dict__,"step": args.steps,"method": method}
    torch.save(ckpt, os.path.join(CKPT_DIR, f"{args.dataset}_{args.tokenizer}_{method}_final.pt"))

    best_ckpt = torch.load(os.path.join(CKPT_DIR, f"{args.dataset}_{args.tokenizer}_{method}_best.pt"), map_location="cpu")
    return {"method": method, "best_step": int(best_ckpt["step"]), "best_val_ppl": float(best_ckpt["val_ppl"]),
...
def summarize_and_plot(methods: List[str], args):
    ...
    fig_path = os.path.join(FIG_DIR, f"{args.dataset}_{args.tokenizer}_val_ppl_vs_step.png")
    ...
    if os.path.exists(log_path):
        ...
        if "spec_c" in df.columns:
            ...
            fig2 = os.path.join(FIG_DIR, f"{args.dataset}_{args.tokenizer}_agop_sc_spectral_concentration.png")
        ...
        if os.path.exists(ck):
            ...
            if os.path.exists(log_path):
                ...
                if args.ppl_threshold > 0:
                    ...
                    if len(S) > 0: step_to_threshold = int(S["step"].iloc[0])
            rows.append({"method": m, "best_step": int(ckpt["step"]),
                         "best_val_ppl": float(ckpt["val_ppl"]), "step_to_threshold": int(step_to_threshold)})
    if rows:
        import pandas as pd
        summary = pd.DataFrame(rows)
        tab_path = os.path.join(TAB_DIR, f"{args.dataset}_{args.tokenizer}_summary.csv")
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="tinyshakespeare", choices=["tinyshakespeare", "tinystories"])
    parser.add_argument("--tokenizer", type=str, default="char", choices=["char", "byte", "gpt2"],
                        help="char only for tinyshakespeare; byte/gpt2 recommended for tinystories")
    parser.add_argument("--data_dir", type=str, default="./data")
    parser.add_argument("--ts_max_chars", type=int, default=2000000)
    ...
    parser.add_argument("--agop_start_step", type=int, default=100)

    parser.add_argument("--topk_logits", type=int, default=0)
    ...
    if args.dataset == "tinyshakespeare":
        if args.tokenizer != "char":
            print("For tinyshakespeare, tokenizer is forced to 'char'.")
            args.tokenizer = "char"
        ...
        if args.tokenizer == "char":
            print("For tinystories, prefer --tokenizer byte or gpt2; continuing with byte.")
            args.tokenizer = "byte"
        train_data, val_data, vocab_size = build_tinystories_dataset(tokenizer=args.tokenizer,
            max_chars=args.ts_max_chars, val_ratio=args.val_ratio, seed=args.seed)
    ...
    with open(os.path.join(EXP_DIR, f"{args.dataset}_{args.tokenizer}_run_summary.json"), "w") as f:
